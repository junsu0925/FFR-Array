...
function [GD, GW] = HKI_Sub_SurPres_Mat_Ls_CHIEF(freq,RealAcuteAngle,c0,rho0,Line_Sec,NN,etaOb,ztaOb)
...
%% CHIF Point
% etaOb = 0.0; % [0.7 0.7 0.7 0.7 0.7 0.7];
% ztaOb = 0.0; % [0.0 0.3 -0.3 0.1 -0.1 0.4];
NumberOfObNode = length(etaOb);
SolidAngleOb = 4*pi*ones(NumberOfObNode,1);
Expanded_TotNumPre = TotNumPre + NumberOfObNode;


etaNode = [etaNode; etaOb];
ztaNode = [ztaNode; ztaOb];
SolidAngle = [SolidAngle; SolidAngleOb];

%% Cal Global D Matrix
% GD(PressureVectorNumber, VelocityVectorNumber)
GD = zeros(Expanded_TotNumPre,TotNumVel);
...
for LineNum = 1 : totNumOfLine
    ...
    parfor num = 1:Expanded_TotNumPre
%      for num = 1:TotNumPre
        % Define eto_o, zta_o
        eta_o = etaNode(num);
        zta_o = ztaNode(num);
...
%% Cal Global W Matrix
GW = zeros(Expanded_TotNumPre,TotNumPre);
GW_UnSum = zeros(Expanded_TotNumPre,TotNumVel); % UnSum
