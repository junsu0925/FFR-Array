...
% SideGab parameter setting
Geometry.gSideValueTable = (0.02 : 0.02 : 0.2)';
% Geometry.gSideValue = (0.02 : 0.005 : 0.2)';
Geometry.gSideValue = 0.08;

% Radiation Impedance Table
% {1,1} = r2, {1.2} = rr, {1,3} = 22, {1,4} = 2r, {1,5} = 12
kaForTable = ((2*pi.*Wave.ff)./Material.sp).*Geometry.a; 
% [RadiationTable,RadiationAreaFactorTable,TableParameter] = RadiationImpTable(Geometry.a, Geometry.RL(1,1), Material.rhowater, Material.sp, kaForTable);
[RadiationTable,RadiationAreaFactorTable,TableParameter] = ...
    RadiationImpTable(Geometry, Material.rhowater, Material.sp, kaForTable);

for ParameterCount = 1 : length(Geometry.gSideValue)
    % side gab input
    Geometry.g(1,1) = Geometry.gSideValue(ParameterCount,1);
...
%     disp('Importing Raiation Impedance Parameters is Finished');
    ...
    [z_RMatrix]=RadiationImp_ParameterStudy_4Array_Subrutine(Material.rhowater,Material.sp,Wave.ka,Geometry.a,Geometry.L,...
        Geometry.RL(1,1),Geometry.gSideValue(ParameterCount,1),Geometry.g(2,1),zlength1,RadiationTable,TableParameter);
    disp('Importing Raiation Impedance Parameters is Finished');
...
%     clear z_RMatrix
    ...
    %% Admittance Plot
    figure(1)
    plot(Wave.f,real(Admittance.Y{1,1}))
    grid on
    ...
    % ka normalized plot
    figure(11)
    plot(Wave.KaRatio(:,1),real(Admittance.Y{1,1}))
    hold off
    grid on
    title('Conductance','fontsize',20, 'fontangle','italic');
    ...
    figure(2)
    plot(Wave.f,real(Admittance.Y{1,2}))
    grid on
    title('Conductance','fontsize',20, 'fontangle','italic');
    xlabel('Frequency [Hz]','fontsize',20, 'fontangle','italic');
    ylabel('Conductance','fontsize',20, 'fontangle','italic');
    ...
    % ka normalized plot
    figure(22)
    plot(Wave.KaRatio(:,1),real(Admittance.Y{1,2}))
    hold off
    grid on
    title('Conductance','fontsize',20, 'fontangle','italic');
    xlabel({'ka normalized by (ka)_0 ','= [ (ka) / (ka)_0 ]     '},'fontsize',20, 'fontangle','italic');
...
SurPlotgaRatio = Geometry.gSideValue/Geometry.a;
